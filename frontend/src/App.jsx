...
import { 
  ...
  ExternalLink, PieChart as PieIcon, Save, Eraser, ArrowRightCircle, Brain, ChevronRight
} from 'lucide-react';
import { 
...
const App = () => {
  ...
  // --- QUIZ LOGIC (UPDATED WITH FEEDBACK) ---
  const startQuiz = async (topic) => {
      ...
      } catch (e) { alert("Could not generate quiz. Try again."); }
  ...
  const loadDateToPlanner = () => {
      if (!calendarTasks || calendarTasks.length === 0) return alert("No tasks to load for this day.");
      ...
      const reconstructedSchedule = {
          ...
          schedule: calendarTasks.map(t => ({
              ...
              task: t.task,
              type: t.type,
              reason: t.reason || "Loaded from Archive",
  ...
  const editCalendarTask = async (taskId, currentText, currentTime) => {
      const newText = window.prompt("Update task name:", currentText);
  ...
  const resetCalendar = async (onlyToday = false) => {
      const msg = onlyToday ? `Clear tasks for ${selectedDate.toDateString()}?` : "⚠️ WIPE ENTIRE CALENDAR?";
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white font-sans flex overflow-hidden selection:bg-purple-500/30">
      <style>{calendarStyle}</style>
      
      <div className="w-20 lg:w-64 bg-black/20 border-r border-white/5 flex flex-col items-center lg:items-start py-8 lg:px-6 gap-8 z-20 backdrop-blur-xl">
         <div className="flex items-center gap-3 mb-4">
             ...
             <div className="hidden lg:block">
                 <span className="block text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-400">Cerebra</span>
                 <p className="text-[10px] text-gray-500 font-medium tracking-widest uppercase -mt-1">Your Syllabus, Strategized.</p>
             </div>
         </div>
         <nav className="flex flex-col w-full gap-2">
             {[{ id: 'planner', icon: LayoutDashboard, label: 'Planner' }, { id: 'stats', icon: PieIcon, label: 'Analytics' }, { id: 'calendar', icon: CalIcon, label: 'Calendar' }, { id: 'history', icon: History, label: 'History' }].map(item => (
                 <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center gap-4 p-3 rounded-xl transition-all w-full group ${activeTab === item.id ? 'bg-purple-600/10 text-purple-400 border border-purple-500/20' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                     <item.icon size={20} className={activeTab === item.id ? 'text-purple-400' : 'text-gray-500 group-hover:text-white'} />
                     <span className="hidden lg:block font-medium">{item.label}</span>
                 </button>
             ))}
         </nav>
         <div className="mt-auto w-full hidden lg:block bg-white/5 p-4 rounded-2xl border border-white/5">
        ...
        <div className="p-8 grid grid-cols-1 xl:grid-cols-3 gap-8">
            <div className="xl:col-span-1 space-y-6">
                ...
                <div className="bg-white/5 border border-white/5 p-6 rounded-3xl space-y-6">
                    ...
                    <div>
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 block">Target Date</label>
                        <input 
                            type="date" 
                            ...
                            onChange={(e) => { 
                                ...
                                if(val) {
                                    ...
                                    const offsetDate = new Date(d.valueOf() + d.getTimezoneOffset() * 60000);
                                    setSelectedDate(offsetDate); 
                                    fetchCalendarTasks(offsetDate);
                                }
                            }} 
                    ...
                    <div>
                        ...
                        {startTimeMode === 'custom' && (
                             <input type="time" value={customStartTime} onChange={e => setCustomStartTime(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white focus:outline-none focus:border-purple-500"/>
                        )}
                    </div>
                                            ...
                                            <div key={idx} className={`p-6 rounded-3xl border transition-all relative overflow-hidden group ${isDone ? 'bg-black/40 border-green-500/20 opacity-60' : isLive ? 'bg-blue-900/10 border-blue-500/50 shadow-[0_0_30px_rgba(59,130,246,0.1)]' : isBreak ? 'bg-emerald-900/5 border-emerald-500/20' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
                                                ...
                                                {isEditing ? (
                                                    ...
                                                    <div className="flex justify-between items-start mb-4">
                                                        ...
                                                        {!isBreak && (<div className="flex gap-2">
                                                            {!isDone && <button onClick={() => startEditing(idx, item)} className="p-2.5 bg-white/5 hover:bg-yellow-500 text-gray-400 hover:text-white rounded-xl transition-all"><Pencil size={18}/></button>}
                                                            <button onClick={() => toggleTaskCompletion(item.task, idx)} className={`p-2.5 rounded-xl transition-all ${isDone ? 'bg-green-500 text-white' : 'bg-white/10 hover:bg-green-500 text-gray-400'}`}>{isDone ? <RefreshCw size={18}/> : <CheckCircle size={18}/>}</button>
                                                            {!isDone && (<button onClick={() => startFocusSession(item)} className="p-2.5 bg-white/10 hover:bg-blue-500 text-gray-400 hover:text-white rounded-xl transition-all"><Timer size={18}/></button>)}
                                                            {!isDone && <button onClick={() => startQuiz(item.task)} title="Take Quiz" className="p-2.5 bg-purple-500/10 hover:bg-purple-600 text-purple-400 hover:text-white rounded-xl transition-all"><Brain size={18}/></button>}
                                                        </div>)}
                                                    </div>
                                                )}
                            ...
                            <Calendar 
                                onChange={(val) => { 
                                    if(val instanceof Date) {
                                        setSelectedDate(val); 
                                        fetchCalendarTasks(val); 
                                    }
                                }} 
                                value={selectedDate instanceof Date ? selectedDate : new Date()} 
                            />
                            <div className="flex justify-between mt-4 pt-4 border-t border-white/5 gap-3">
                ...
                <div className="bg-[#1e293b] w-full max-w-lg rounded-3xl p-8 border border-white/10 shadow-2xl relative">
                    ...
                    {quizLoading ? (
                        <div className="flex flex-col items-center py-12">
                            <Sparkles className="animate-spin text-purple-500 mb-4" size={48}/>
                            <p className="text-purple-200 font-bold animate-pulse">Generating Quiz...</p>
                        </div>
                    ) : showQuizResult ? (
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-6">
                                <span className="text-xs font-bold text-purple-400 tracking-wider">QUESTION {currentQuestion + 1}/{quizData.length}</span>
                                <span className="text-xs font-bold text-gray-500">Score: {quizScore}</span>
                            </div>
                            ...
                            <div className="space-y-3">
                                {quizData[currentQuestion].options.map((opt, i) => {
                                    ...
                                    // LOGIC: Highlighting Correct/Wrong Answers
                                    if (selectedAnswer) {
                                        if (opt === quizData[currentQuestion].answer) {
                                            bgClass = "bg-green-500/20 border-green-500 text-green-300 shadow-[0_0_15px_rgba(34,197,94,0.3)]";
                                    ...
                                    return (
                                        <button 
                                            ...
                                            className={`w-full text-left p-4 rounded-xl border border-white/5 transition-all text-sm font-medium ${bgClass}`}
